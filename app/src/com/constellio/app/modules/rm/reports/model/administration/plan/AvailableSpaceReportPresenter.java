...
public class AvailableSpaceReportPresenter {
    ...
    public AvailableSpaceReportModel build() {
        ...
        if (rootStorageSpaces != null) {
            ...
            for (Record record : rootStorageSpaces) {
                parent.setCode(record.getTypeCode()).setTitle(record.getTitle()).setAvailableSpace(10);
                List<Record> childStorageSpaces = conceptNodesTaxonomySearchServices.getChildConcept(record, new TaxonomiesSearchOptions().setRows(10000));
                for (Record record1 : childStorageSpaces) {
                    child.setCode(record1.getSchemaCode()).setTitle(record1.getTitle()).setAvailableSpace(100);
