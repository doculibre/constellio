...
public class RMMigrationTo6_7 implements MigrationScript {
    ...
    public static class SchemaAlterationsFor6_7 extends MetadataSchemasAlterationHelper {
        ...
        protected void migrate(MetadataSchemaTypesBuilder typesBuilder) {
            typesBuilder.getDefaultSchema(ContainerRecord.DEFAULT_SCHEMA).create(ContainerRecord.LINEAR_SIZE_ENTERED)
                    .setType(MetadataValueType.NUMBER).setEssential(false).setUndeletable(true);
            typesBuilder.getDefaultSchema(ContainerRecord.DEFAULT_SCHEMA).create(ContainerRecord.LINEAR_SIZE_SUM)
                    .setType(MetadataValueType.NUMBER).setEssential(false).setUndeletable(true);
            typesBuilder.getDefaultSchema(ContainerRecord.DEFAULT_SCHEMA).create(ContainerRecord.LINEAR_SIZE)
                    .setType(MetadataValueType.NUMBER).setEssential(false).setUndeletable(true)
