...
public abstract class SipXmlUtils {
	/**
	...
	public SipXmlUtils(AppLayerFactory appLayerFactory, String collection, Locale locale) {
		...
		if (metadata.getLocalCode().toLowerCase().contains("copyrule")) {
			...
			for (ModifiableStructure value : metadataValue) {
				...
				listOfMetadataTags.addAll(asList(
						...
						new Element(metadata.getLocalCode() + "_active_period")
								.setText(retentionRule.getActiveRetentionPeriod().toString()),
						new Element(metadata.getLocalCode() + "_active_period_comment")
								.setText(retentionRule.getActiveRetentionComment()),
						new Element(metadata.getLocalCode() + "_semi_active_period")
								.setText(retentionRule.getSemiActiveRetentionPeriod().toString()),
						new Element(metadata.getLocalCode() + "_semi_active_period_comment")
	...
	public static List<Element> createMetadataTagFromMetadataOfTypeReference(Metadata metadata, Record recordElement,
		...
		if (listOfRecordReferencedByMetadata.isEmpty()) {
			String elementNamePrefix = metadata.getLocalCode();//  REFERENCE_PREFIX + inheritedMetadataCode.replace("_default_", "_");
			...
			String elementNamePrefix = metadataInSchema.getLocalCode();
			listOfMetadataTags.addAll(asList(
