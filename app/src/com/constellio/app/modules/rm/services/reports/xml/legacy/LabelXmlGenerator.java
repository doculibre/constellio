...
import com.constellio.app.modules.rm.services.reports.xml.legacy.parameters.AbstractXmlGeneratorParameters;
...
public class LabelXmlGenerator extends AbstractXmlGenerator {
	...
	public LabelXmlGenerator(String collection, AppLayerFactory appLayerFactory, Locale locale, String username) {
		...
		this.user = username == null ? null :
					appLayerFactory.getModelLayerFactory().newUserServices().getUserInCollection(username, collection);
	}

	public LabelXmlGenerator(String collection, AppLayerFactory appLayerFactory, Locale locale, String username,
							 Record... recordElements) {
		this(collection, appLayerFactory, locale, username);
	...
	public LabelXmlGenerator(String collection, AppLayerFactory appLayerFactory, Locale locale, int startingPosition,
							 int numberOfCopies, String username, Record... recordElements) {
		this(collection, appLayerFactory, locale, username, recordElements);
	...
	private List<Element> createMetadataTagsFromMetadata(Metadata metadata, Record recordElement) {
		...
		if (metadata.isSeparatedStructure()) {
			return createMetadataTagFromMetadataOfTypeSeparatedStructure(metadata, recordElement);
		}
	...
	public static List<Element> createMetadataTagFromMetadataOfTypeSeparatedStructure(Metadata metadata,
																					  Record recordElement) {
		List<Element> listOfMetadataTags = new ArrayList<>();
		String mainStructureValue = null;
		if ((mainStructureValue = recordElement.get(metadata, GetMetadataOption.ONLY_MAIN_STRUCTURE_VALUE)) != null) {
			listOfMetadataTags.add(new Element(escapeForXmlTag(getLabelOfMetadata(metadata))).setText(mainStructureValue));
		}
