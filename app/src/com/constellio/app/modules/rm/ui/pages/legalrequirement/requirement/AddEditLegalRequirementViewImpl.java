...
public class AddEditLegalRequirementViewImpl extends BaseViewImpl implements AddEditLegalRequirementView {

	private static Logger LOGGER = LoggerFactory.getLogger(AddEditLegalRequirementViewImpl.class);

	private AddEditLegalRequirementPresenter presenter;

	public AddEditLegalRequirementViewImpl() {
		presenter = new AddEditLegalRequirementPresenter(this);
	}
	...
	protected String getTitle() {
		return $("LegalRequirementManagement.addEditLegalRequirement");
	}
	...
	private Component buildRecordForm() {
		return new RecordForm(presenter.getRecordVO(), new MetadataFieldFactory(), ConstellioFactories.getInstance()) {
			...
			protected boolean isDirty(Record record) {
				return super.isDirty(record) || presenter.isRequirementsReferenceDirty();
			}
		};
	}
}
