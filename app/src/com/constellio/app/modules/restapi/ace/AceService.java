package com.constellio.app.modules.restapi.ace;
...
public class AceService extends BaseService {
    ...
    public boolean updateAces(User user, Record record, List<AceDto> aces) {
        ...
        if (!acesToAdd.isEmpty()) aceDao.addAces(user, record, acesToAdd);
        if (!acesToModify.isEmpty()) aceDao.updateAces(user, record, acesToModify);
        if (!acesToRemove.isEmpty()) aceDao.removeAces(user, record, acesToRemove);
