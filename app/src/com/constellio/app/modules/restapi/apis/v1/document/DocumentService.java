...
public class DocumentService extends ResourceService {
	...
	public DocumentDto merge(String host, String id, String serviceKey, String method, String date,
							 int expiration, String signature, DocumentDto document, List<String> mergeSourceIds,
							 String flushMode, Set<String> filters, boolean urlValidated) throws Exception {
		return merge(host, id, serviceKey, method, date, expiration, signature, document,
				mergeSourceIds, flushMode, filters, urlValidated, null);
	}

	public DocumentDto merge(String host, String id, String serviceKey, String method, String date, int expiration,
							 String signature, DocumentDto document,
							 List<String> mergeSourceIds,
							 String flushMode, Set<String> filters, boolean urlValidated, Boolean partial)
			throws Exception {

		if (partial == null) {
			return mergeCreateDocument(host, id, serviceKey, method, date, expiration, signature, document,
					mergeSourceIds, flushMode, filters, urlValidated);
		} else {
			return mergeUpdateDocument(host, id, serviceKey, method, date, expiration, signature, document,
					mergeSourceIds, flushMode, filters, urlValidated, Boolean.TRUE.equals(partial));
		}
	}

	private DocumentDto mergeCreateDocument(String host, String folderId, String serviceKey, String method, String date,
											int expiration, String signature, DocumentDto document,
											List<String> mergeSourceIds, String flushMode, Set<String> filters,
											boolean urlValidated) throws Exception {
		validateParameters(host, folderId, serviceKey, method, date, expiration, null, null, null, signature, urlValidated);
	...
	private DocumentDto mergeUpdateDocument(String host, String id, String serviceKey, String method, String date,
											int expiration, String signature, DocumentDto document,
											List<String> mergeSourceIds, String flushMode, Set<String> filters,
											boolean urlValidated, boolean partial) throws Exception {
		return setDocumentContentAndUpdate(host, id, serviceKey, method, date, expiration, signature, document,
				null, mergeSourceIds, partial, flushMode, filters, urlValidated, true);
	}
	...
	private void validateDocument(String host, String folderId, String serviceKey, String method, String date,
		...
		validateDocumentExtendedAttributesAndAuthorizations(document, collection, schema);
	}

	private void validateDocumentExtendedAttributesAndAuthorizations(DocumentDto document, String collection,
	...
	private DocumentDto updateDocument(DocumentDto document, Record documentRecord, Content content, String flushMode,
									   Set<String> filters,
									   User user, MetadataSchema schema, boolean partial) throws Exception {
		try {
			boolean acesModified = false;
			documentRecord = documentDao.updateDocument(user, documentRecord, schema, document, content, partial, flushMode);
			...
			return getAdaptor().adapt(document, documentRecord, schema, acesModified, filters);
		} catch (Exception e) {
			if (content != null) {
				documentDao.deleteContent(content);
			}
	...
	public DocumentDto update(String host, String id, String serviceKey, String method, String date, int expiration,
							  ...
							  String flushMode, Set<String> filters, boolean urlValidated) throws Exception {
		return setDocumentContentAndUpdate(host, id, serviceKey, method, date, expiration, signature, document,
				contentInputStream, null, partial, flushMode, filters, urlValidated, false);
	}

	private DocumentDto setDocumentContentAndUpdate(String host, String id, String serviceKey, String method,
													String date, int expiration, String signature, DocumentDto document,
													InputStream contentInputStream, List<String> mergeSourceIds,
													boolean partial, String flushMode, Set<String> filters,
													boolean urlValidated, boolean merging) throws Exception {
		...
		String collection = documentRecord.getCollection();
		User user = getUserByServiceKey(serviceKey, collection);
		...
		if (partial && document.getType() == null) {
			...
			documentSchema = documentDao.getResourceMetadataSchema(documentTypeRecord, collection);
		} else {
			documentSchema = documentDao.getLinkedMetadataSchema(document.getType(), collection);
		}

		validateDocumentExtendedAttributesAndAuthorizations(document, collection, documentSchema);
		...
		if (merging) {
			...
			return updateDocument(document, getRecord(id, true), content,
					flushMode, filters, user, documentSchema, Boolean.TRUE.equals(partial));
		} else {
			Content content = null;
			if (document.getContent() != null) {
			...
			return updateDocument(document, documentRecord, content, flushMode, filters, user, documentSchema, partial);
		}
	}
