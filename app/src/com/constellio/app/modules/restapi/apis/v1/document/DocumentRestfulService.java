...
public class DocumentRestfulService extends ResourceRestfulService {
	...
	public Response create(
		...
		List<String> documentIdsToMerge = getDocumentIdsToMerge(mergeSourceIds);
	...
	public Response update(@Parameter(required = true, description = "Document Id") @QueryParam("id") String id,
						   ...
						   @Parameter(description = "A document id list can be specified to activate the merge mode.<br>" +
													"The id list must be provided as a string without space and each id separated by a comma.<br>" +
													"The document will be updated with the result of merging all documents provided in the list.<br>" +
													"If the document's id is not in the list, his content won't be merged with the other documents in the list")
							   @HeaderParam(CustomHttpHeaders.MERGE_SOURCE) String mergeSourceIds,
		...
		DocumentDto updatedDocument = CollectionUtils.isEmpty(documentIdsToMerge) ?
									  documentService.update(host, id, serviceKey, method, date, expiration, signature,
											  document, fileStream, false, flush, filters, isUrlValidated()) :
									  documentService.merge(host, id, serviceKey, method, date, expiration, signature,
											  document, documentIdsToMerge, flush, filters, isUrlValidated(), Boolean.FALSE);

		return Response.ok(updatedDocument).tag(updatedDocument.getETag()).build();
	}

	@Nullable
	private List<String> getDocumentIdsToMerge(String mergeSourceIds) {
		validateMergeSourceIds(mergeSourceIds);
		return StringUtils.isBlank(mergeSourceIds) ? null : Arrays.asList(mergeSourceIds.split(","));
	}

	private void validateMergeSourceIds(String mergeSourceIds) {
		if (StringUtils.isNotBlank(mergeSourceIds) && mergeSourceIds.indexOf(' ') != -1) {
			throw new InvalidParameterException(CustomHttpHeaders.MERGE_SOURCE, mergeSourceIds);
		}
	}
	...
	public Response updatePartial(@Parameter(required = true, description = "Document Id") @QueryParam("id") String id,
		...
		DocumentDto updatedDocument = CollectionUtils.isEmpty(documentIdsToMerge) ?
									  ...
									  documentService.merge(host, id, serviceKey, method, date, expiration, signature,
											  document, documentIdsToMerge, flush, filters, isUrlValidated(), Boolean.TRUE);
