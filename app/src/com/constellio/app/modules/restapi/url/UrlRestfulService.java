...
public class UrlRestfulService {
    ...
    public Response get(@Parameter(required=true, description="Token") @QueryParam("token") String token,
        ...
        if (schemaType == null) throw new RequiredParameterException("schemaType");
        ...
        if (!HttpMethods.contains(method)) throw new InvalidParameterException("method", method);
        ...
        if (Strings.isNullOrEmpty(folderId) && method.equals(POST)) throw new RequiredParameterException("folderId");
        if (!Strings.isNullOrEmpty(id) && method.equals(POST)) throw new InvalidParameterWithHttpMethodException("id", method);

        if (!Strings.isNullOrEmpty(physical) && !method.equals(DELETE)) throw new InvalidParameterWithHttpMethodException("physical", method);

        if (!Strings.isNullOrEmpty(version) && !method.equals(GET)) throw new InvalidParameterWithHttpMethodException("version", method);
