...
public class ValidationService extends BaseService {
    ...
    public void validateUserAccess(User user, Record record, String method) {
        switch (method) {
            case GET:
                if (!user.hasReadAccess().on(record)) throw new UnauthorizedAccessException();
                break;
            case POST:
            case PUT:
            case PATCH:
                if (!user.hasWriteAccess().on(record)) throw new UnauthorizedAccessException();
                break;
            case DELETE:
                if (!user.hasDeleteAccess().on(record)) throw new UnauthorizedAccessException();
                break;
            default:
        ...
        for (AceDto ace : ListUtils.nullToEmpty(authorizations)) {
            for (String principal : ace.getPrincipals()) {
                ...
                if (added) {
                    ...
                    if (record == null) throw new RecordNotFoundException(principal);
                    if (isLogicallyDeleted(record)) throw new RecordLogicallyDeletedException(principal);
                }
            }

            if (ace.getEndDate() != null && ace.getStartDate() == null) throw new RequiredParameterException("ace.startDate");
            if (ace.getStartDate() != null && ace.getEndDate() != null) {
                ...
                if (end.isBefore(start)) throw new InvalidDateCombinationException(ace.getStartDate(), ace.getEndDate());
            }
        }
    }
