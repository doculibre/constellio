...
public class DocumentRestfulServiceV2AcceptanceTest extends BaseRestfulServiceV2AcceptanceTest {
	...
	public void givenIdThenSummaryRecordReturned() {
		...
		assertThat(recordDto.getSchemaType()).isEqualTo(Document.SCHEMA_TYPE);
		assertThat(recordDto.getMetadatas()).isNotEmpty();

		boolean hasNonSummaryMetadata = recordDto.getMetadatas().keySet().stream()
				.anyMatch(metadataCode -> !SchemaUtils.isSummary(rm.defaultDocumentSchema().get(metadataCode)));
		assertThat(hasNonSummaryMetadata).isFalse();

		Record documentRecord = recordServices.getDocumentById(recordId);
	...
	private Response doGetByIdQuery(String id, String authHeaderValue, FilterMode filterMode) {
		WebTarget target = newWebTarget("v2/documents/{id}").resolveTemplate("id", id);
