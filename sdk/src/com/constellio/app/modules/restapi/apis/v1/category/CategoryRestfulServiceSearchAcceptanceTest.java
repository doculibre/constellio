...
public class CategoryRestfulServiceSearchAcceptanceTest extends BaseRestfulServiceAcceptanceTest {

	private String searchExpression = "x13";
	...
	public void setUp() throws Exception {
		...
		webTarget = newWebTarget("v1/categories/search", new ObjectMapper());
	...
	public void whenCallingService() {
		Response response = webTarget.queryParam("serviceKey", adminServiceKey)
				.queryParam("collection", zeCollection)
				.queryParam("expression", searchExpression).request()
				.header(HttpHeaders.HOST, host).header(HttpHeaders.AUTHORIZATION, "Bearer ".concat(adminToken))
				.get();
		...
		List<CategoryDto> categories = response.readEntity(newCategoryList());
	...
	public void whenCallingServiceWithHighResultCount() {
		Response response = webTarget.queryParam("serviceKey", adminServiceKey)
				.queryParam("collection", zeCollection)
				.queryParam("expression", "Z").request()
				.header(HttpHeaders.HOST, host).header(HttpHeaders.AUTHORIZATION, "Bearer ".concat(adminToken))
				.get();
		...
		List<CategoryDto> categories = response.readEntity(newCategoryList());
		assertThat(categories).isNotNull();
		assertThat(categories.size()).isEqualTo(autoCompleteSize);
	}
	...
	private GenericType<List<CategoryDto>> newCategoryList() {
		return new GenericType<List<CategoryDto>>() {
		};
	}
}
