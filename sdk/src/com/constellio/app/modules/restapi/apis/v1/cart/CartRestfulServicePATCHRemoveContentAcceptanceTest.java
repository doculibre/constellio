...
public class CartRestfulServicePATCHRemoveContentAcceptanceTest extends BaseRestfulServiceAcceptanceTest {
	private RolesManager rolesManager;
	private CartUtil cartUtil;

	private Cart cart;
	private CartContentUpdateDto contentToRemove;
	...
	public void setUp() throws Exception {
		...
		cart = createCart();
		contentToRemove = CartContentUpdateDto.builder().itemsToRemove(asList(records.folder_A01, records.folder_A01, records.folder_B02)).build();
	...
	public void validateServiceWithMyCartId()
		...
		contentToRemove = CartContentUpdateDto.builder().itemsToRemove(asList(records.folder_A10)).build();
		Response response = webTarget.queryParam("serviceKey", serviceKey)
				...
				.build("PATCH", entity(contentToRemove, APPLICATION_JSON_TYPE)).invoke();
		...
		folder = rm.getFolder(records.folder_A10);
	...
	@Test
	public void validateServiceWithoutReadWritePermissionOnFolder() throws Exception {
	...
	public void whenCallingServiceWithInvalidContent() {
		contentToRemove = CartContentUpdateDto.builder().itemsToAdd(asList("fakeId")).build();
		Response response = webTarget.queryParam("serviceKey", serviceKey)
				.queryParam("id", cart.getId()).request()
				.header(HttpHeaders.HOST, host).header(HttpHeaders.AUTHORIZATION, "Bearer ".concat(token))
				.build("PATCH", entity(contentToRemove, APPLICATION_JSON_TYPE)).invoke();
