...
public class ConstellioHeaderPresenterAcceptanceTest extends ConstellioTest {
    ...
    @Test
    public void givenMetadataWithoutHeritanceButInTwoDifferentCustomSchemasThenShowsUpInAdvancedSearchIndependently() throws RecordServicesException {
        connectWithAdmin();
        getModelLayerFactory().getMetadataSchemasManager().modify(zeCollection, new MetadataSchemaTypesAlteration() {
            @Override
            public void alter(MetadataSchemaTypesBuilder types) {
        ...
        SchemasDisplayManager manager = getAppLayerFactory().getMetadataSchemasDisplayManager();
        manager.saveMetadata(manager.getMetadata(zeCollection, "folder_mySchema_myMetadata").withVisibleInAdvancedSearchStatus(true));
        manager.saveMetadata(manager.getMetadata(zeCollection, "folder_mySchema2_myMetadata").withVisibleInAdvancedSearchStatus(true));
        List<MetadataVO> allowedMetadatas = presenter.getMetadataAllowedInCriteria();
        assertThat(allowedMetadatas).extracting("code").doesNotContain("folder_mySchema_myMetadata", "folder_mySchema2_myMetadata");
