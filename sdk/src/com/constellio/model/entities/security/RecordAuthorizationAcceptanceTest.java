...
public class RecordAuthorizationAcceptanceTest extends ConstellioTest {
	...
	public void givenAuthorizationWithTimeRangeThenActiveAtTheRightTime()
		...
		RecordAuthorization auth = (RecordAuthorization) schemas.newAuthorization();
	...
	public List<String> getReadRecordsForUser(User user) {
		...
		return searchServices.searchRecordIds(new LogicalSearchQuery().setCondition(
				from(type).returnAll()).filteredWithUserRead(user).sortAsc(Schemas.IDENTIFIER));
	}
