...
public class RecordServicesAgregatedRefCountIntergerMetadatasAcceptTest extends ConstellioTest {
	...
	public void givenReferenceCountAggregatedMetadatasThenCalculatedAccurately()
		...
		defineSchemasManager().using(schemas.with(new MetadataSchemaTypesConfigurator() {
			...
			public void configure(MetadataSchemaTypesBuilder schemaTypes) {
				...
				MetadataBuilder anotherSchema_refCount = anotherType.getDefaultSchema().create("refCount")
						.setType(INTEGER).defineDataEntry().asReferenceCount(zeSchema_zeRef);
				...
				MetadataBuilder thirdSchema_refCount = thirdType.getDefaultSchema().create("refCount")
						.setType(INTEGER).defineDataEntry().asReferenceCount(anotherSchema_zeRef);
		...
		waitForBatchProcess();
		assertThat(record("merge1").<Integer>get(anotherSchema_refCount)).isEqualTo(2);
		assertThat(record("merge2").<Integer>get(anotherSchema_refCount)).isEqualTo(3);
		assertThat(record("merge3").<Integer>get(thirdSchema_refCount)).isEqualTo(2);

		tx = new Transaction();
		tx.add(r3.set("ref", "merge1"));
		getModelLayerFactory().newRecordServices().execute(tx);
		waitForBatchProcess();
		assertThat(record("merge1").<Integer>get(anotherSchema_refCount)).isEqualTo(3);
		assertThat(record("merge2").<Integer>get(anotherSchema_refCount)).isEqualTo(2);
		assertThat(record("merge3").<Integer>get(thirdSchema_refCount)).isEqualTo(2);
		...
		getModelLayerFactory().newRecordServices().execute(tx);
		waitForBatchProcess();
		assertThat(record("merge1").<Integer>get(anotherSchema_refCount)).isEqualTo(5);
		assertThat(record("merge2").<Integer>get(anotherSchema_refCount)).isEqualTo(0);
		assertThat(record("merge3").<Integer>get(thirdSchema_refCount)).isEqualTo(2);
