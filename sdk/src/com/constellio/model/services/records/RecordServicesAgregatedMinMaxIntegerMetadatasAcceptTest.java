...
public class RecordServicesAgregatedMinMaxIntegerMetadatasAcceptTest extends ConstellioTest {
	...
	public void givenMinOfMultiValueNumberMetadatasThenThenCalculatedAccurately()
		...
		defineSchemasManager().using(schemas.with(new MetadataSchemaTypesConfigurator() {
			...
			public void configure(MetadataSchemaTypesBuilder schemaTypes) {
				...
				MetadataBuilder zeSchema_valueMetadata = zeType.getDefaultSchema().create("numberValue").setType(INTEGER)
						.setMultivalue(true);
				...
				MetadataBuilder anotherSchema_minValue = anotherType.getDefaultSchema().create("minValue")
						.setType(INTEGER).defineDataEntry().asMin(zeSchema_zeRef, zeSchema_valueMetadata);
				MetadataBuilder anotherSchema_maxValue = anotherType.getDefaultSchema().create("maxValue")
						.setType(INTEGER).defineDataEntry().asMax(zeSchema_zeRef, zeSchema_valueMetadata);
				...
				MetadataBuilder thirdSchema_minValue = thirdType.getDefaultSchema().create("minValue")
						.setType(INTEGER).defineDataEntry().asMin(anotherSchema_zeRef, anotherSchema_minValue);
				MetadataBuilder thirdSchema_maxValue = thirdType.getDefaultSchema().create("maxValue")
						.setType(INTEGER).defineDataEntry().asMax(anotherSchema_zeRef, anotherSchema_maxValue);
		...
		tx = new Transaction();
		TestRecord r1 = new TestRecord(zeSchema, "r1").set("numberValue", asList(107, 114)).set("ref", "merge1");
		TestRecord r2 = new TestRecord(zeSchema, "r2").set("numberValue", asList(109, 105)).set("ref", "merge1");
		TestRecord r3 = new TestRecord(zeSchema, "r3").set("numberValue", asList(101, 116)).set("ref", "merge2");
		TestRecord r4 = new TestRecord(zeSchema, "r4").set("numberValue", asList(102, 115)).set("ref", "merge2");
		TestRecord r5 = new TestRecord(zeSchema, "r5").set("numberValue", asList(103, 114)).set("ref", "merge2");
		getModelLayerFactory().newRecordServices().execute(tx.addAll(r1, r2, r3, r4, r5));
		waitForBatchProcess();
		assertThat(record("merge1").<Integer>get(anotherSchema_minValue)).isEqualTo(105);
		assertThat(record("merge2").<Integer>get(anotherSchema_minValue)).isEqualTo(101);
		assertThat(record("merge3").<Integer>get(thirdSchema_minValue)).isEqualTo(101);
		assertThat(record("merge1").<Integer>get(anotherSchema_maxValue)).isEqualTo(114);
		assertThat(record("merge2").<Integer>get(anotherSchema_maxValue)).isEqualTo(116);
		assertThat(record("merge3").<Integer>get(thirdSchema_maxValue)).isEqualTo(116);

		tx = new Transaction();
		tx.add(r1.set("numberValue", asList(107, 5)));
		tx.add(r3.set("ref", "merge1"));
		getModelLayerFactory().newRecordServices().execute(tx);
		waitForBatchProcess();
		assertThat(record("merge1").<Integer>get(anotherSchema_minValue)).isEqualTo(5);
		assertThat(record("merge2").<Integer>get(anotherSchema_minValue)).isEqualTo(102);
		assertThat(record("merge3").<Integer>get(thirdSchema_minValue)).isEqualTo(5);
		assertThat(record("merge1").<Integer>get(anotherSchema_maxValue)).isEqualTo(116);
		assertThat(record("merge2").<Integer>get(anotherSchema_maxValue)).isEqualTo(115);
		assertThat(record("merge3").<Integer>get(thirdSchema_maxValue)).isEqualTo(116);
	...
	public void whenComparingValuesOfMultipleTypesThenRetrieveMinAndMax()
		...
		values = asList((Object) 12, 1, 40, 44);
		assertThat(MinMetadataAggregationHandler.getMin(INTEGER, values)).isEqualTo(1);
		assertThat(MaxMetadataAggregationHandler.getMax(INTEGER, values)).isEqualTo(44);
