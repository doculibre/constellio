...
public class BorrowingServicesAcceptTest extends ConstellioTest {
	...
	public void givenSemiActiveFolderWhenBorrowItThenOk()
		...
		assertThat(folderC30.getBorrowUser()).isEqualTo(rm.getAdmin().getId());
		assertThat(folderC30.getBorrowUserEntered()).isEqualTo(rm.getAdmin().getId());
	...
	public void givenInactiveFolderWhenBorrowItThenOk()
		...
		Folder folderA94 = rm.getFolder_A94();

		borrowingServices.borrowFolder(folderA94.getId(), previewReturnDate, rm.getAdmin(), rm.getAlice());
		folderA94 = rm.getFolder_A94();
		...
		assertThat(folderA94.getBorrowUser()).isEqualTo(rm.getAdmin().getId());
		assertThat(folderA94.getBorrowUserEntered()).isEqualTo(rm.getAlice().getId());
		assertThat(searchServices.getResultsCount(rmEventsSearchServices.newFindCurrentlyBorrowedFoldersQuery(rm.getAdmin())))
				.isEqualTo(1);
	}
	...
	public void givenActiveFolderWhenBorrowItThenCannotBorrow()
		...
		Date previewReturnDate = nowDateTime.plusDays(15).toDate();
		Folder folderA16 = rm.getFolder_A16();

		try {
			borrowingServices.borrowFolder(folderA16.getId(), previewReturnDate, rm.getAdmin(), rm.getAdmin());
		} finally {
			folderA16 = rm.getFolder_A16();
	...
	public void givenBorrowedFolderWhenBorrowItThenCannotBorrow()
		...
		Date previewReturnDate = nowDateTime.plusDays(15).toDate();
		Folder folderC30 = rm.getFolder_C30();
		borrowingServices.borrowFolder(folderC30.getId(), previewReturnDate, rm.getAdmin(), rm.getAdmin());

		try {
			borrowingServices.borrowFolder(folderC30.getId(), previewReturnDate, rm.getAdmin(), rm.getAdmin());
		} finally {
			folderC30 = rm.getFolder_C30();
			assertThat(folderC30.getArchivisticStatus()).isEqualTo(FolderStatus.SEMI_ACTIVE);
			assertThat(folderC30.getBorrowed()).isTrue();
			assertThat(folderC30.getBorrowDate()).isEqualTo(nowDateTime);
			assertThat(folderC30.getBorrowReturnDate()).isNull();
	...
	public void whenReturnFolderThenOk()
		...
		borrowingServices.returnFolder(folderC30.getId(), rm.getAdmin());
		folderC30 = rm.getFolder_C30();
		...
		assertThat(event.getUsername()).isEqualTo(rm.getAdmin().getUsername());
	...
	public void givenAdminUserBorrowFolderToUserWhenHeReturnsFolderThenOk()
		...
		Date previewReturnDate = nowDateTime.plusDays(15).toDate();
		borrowingServices
				.borrowFolder(rm.getFolder_C30().getId(), previewReturnDate, rm.getAdmin(), rm.getBob_userInAC());
		Folder folderC30 = rm.getFolder_C30();

		givenTimeIs(nowDateTime.plusDays(1));
		borrowingServices.returnFolder(folderC30.getId(), rm.getBob_userInAC());
		folderC30 = rm.getFolder_C30();
		...
		assertThat(records).hasSize(1);
		Event event = new Event(records.get(0), getSchemaTypes());
		assertThat(event.getUsername()).isEqualTo(rm.getBob_userInAC().getUsername());
		assertThat(event.getType()).isEqualTo(EventType.RETURN_FOLDER);
		assertThat(event.getCreatedOn()).isEqualTo(nowDateTime.plusDays(1));
	}
	...
	public void givenAdminUserBorrowFolderToBobWhenAdminReturnsFolderThenOk()
		...
		Date previewReturnDate = nowDateTime.plusDays(15).toDate();
		borrowingServices.borrowFolder(rm.getFolder_C30().getId(), previewReturnDate, rm.getAdmin(), rm.getBob_userInAC());
		Folder folderC30 = rm.getFolder_C30();
	...
	private void givenBorrowedFolderC30ByAdmin()
		...
		Date previewReturnDate = nowDateTime.plusDays(15).toDate();
		borrowingServices.borrowFolder(rm.getFolder_C30().getId(), previewReturnDate, rm.getAdmin(), rm.getAdmin());
	}
