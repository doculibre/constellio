...
public class FileSystemConfigManagerUnitTest extends ConstellioTest {
	...
	public void setUp() {
		...
		xmlConfiguration = new XMLConfiguration(hashOfAllFile, document);
	...
	public void whenAddNewBinaryThenBinaryAdded()
		...
		doReturn(binaryConfigurationFile).when(configManager).createFoldersAndFiles(anyString());

		configManager.add(binaryConfigurationPath, inputStream);

		verify(ioServices).copy(inputStream, binaryOutputStream);
	}
	...
	public void whenUpdateBinaryThenBinaryUpdated()
		...
		when(ioServices.newFileOutputStream(binaryConfigurationFile, FileSystemConfigManager.UPDATE_BINARY_FILE))
				.thenReturn(binaryOutputStream);
