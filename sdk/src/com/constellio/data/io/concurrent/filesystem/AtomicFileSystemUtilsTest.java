...
//TODO check why tests were disabled 
@Ignore
public class AtomicFileSystemUtilsTest {
	...
	public void setUp(){
		...
		File masterDirectory = new File(baseFld, "master");
		File slaveDirectory = new File(baseFld, "slave");
		assertThat(masterDirectory.exists()).isEqualTo(false);
		assertThat(slaveDirectory.exists()).isEqualTo(false);
		master = new ChildAtomicFileSystem(new AtomicLocalFileSystem(hashingService), masterDirectory.getAbsolutePath());
		slave = spy(new ChildAtomicFileSystem(new AtomicLocalFileSystem(hashingService), slaveDirectory.getAbsolutePath()));
