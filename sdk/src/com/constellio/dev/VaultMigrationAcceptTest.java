...
//        TODO Francis : givenDocumentsWithContentWhenMigrateVaultThenContentsExistInVault ne roule pas avec prepare system
    ...
    public void createPrintable() throws Exception {
        File file;
        Content content;
        List<Record> recordsList = new ArrayList<>();
        for (int index =1; index<=SIZE_OF_CREATED_CONTENTS; index++){
    ...
    public void createDecommissioningLists() throws Exception {
        ...
        for (int index =1; index<=SIZE_OF_CREATED_CONTENTS; index++){
            ...
            documentReport = newTempFileWithContent("decommissioningListDocument"+ index +".txt", "This is content of document report " + index);
            folderReport = newTempFileWithContent("decommissioningListFolder"+ index +".txt", "This is content of folder report " + index);
            documentReportContent = contentManager.createMajor(records.getAdmin(), documentReport.getName(), contentManager.upload(documentReport));
            folderReportContent = contentManager.createMajor(records.getAdmin(), folderReport.getName(), contentManager.upload(folderReport));
    ...
    public void createTemporaryRecord() throws Exception {
        File file;
        Content content;
        List<Record> recordsList = new ArrayList<>();
        for (int index =1; index<=SIZE_OF_CREATED_CONTENTS; index++){
    ...
    @Test
    public void givenDocumentsWithContentWhenImproveHashCodesThenHashesAreReplaced() throws Exception {
        createDocuments();
    ...
    @Test
    public void givenDocumentsWithContentWhenMigrateVaultThenContentsExistInVault() throws Exception {
        createDocuments();
        MetadataSchemaTypes metadataSchemaTypes = getModelLayerFactory().getMetadataSchemasManager().getSchemaTypes("zeCollection");
        MetadataSchemaType schemaType = getAppLayerFactory().getModelLayerFactory().getMetadataSchemasManager()
                .getSchemaTypes("zeCollection").getSchemaType("document");
        ...
        for(Record record : newRecords){
            Document document = new Document(record, metadataSchemaTypes);
            InputStream inputStream = getModelLayerFactory().getContentManager().getContentInputStream(document.getContent().getCurrentVersion().getHash(),document.getTitle());
            IOServices ioServices = getDataLayerFactory().getIOServicesFactory().newIOServices();
    ...
    @Test
    public void givenDocumentsWithContentWithOtherVersionsWhenMigrateVaultThenHashesAreCorrectlyReplaced() throws Exception {
        createDocuments();
        ...
        ContentVersionDataSummary contentVersionDataSummary = contentManager.getContentVersionSummary(content.getCurrentVersion().getHash()).getContentVersionDataSummary();
    ...
    @Test
    public void givenDocumentsWithContentWhenMigrateVaultThenHashesAreCorrectlyReplaced() throws Exception {
        createDocuments();
        MetadataSchemaTypes metadataSchemaTypes = getModelLayerFactory().getMetadataSchemasManager().getSchemaTypes("zeCollection");
        MetadataSchemaType schemaType = getAppLayerFactory().getModelLayerFactory().getMetadataSchemasManager()
                .getSchemaTypes("zeCollection").getSchemaType("document");
