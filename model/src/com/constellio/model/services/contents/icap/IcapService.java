    ...
    void readPreviewLengthFromIcapServer() {
        if (modelLayerFactory.getSystemConfigs().getIcapScanActivated()) {
            ...
            if (StringUtils.isBlank(icapServerUrl)) {
                ...
                try {
                    icapPreviewLengthNewValue = new IcapClient(URI.create(icapServerUrl), icapResponseTimeout).getIcapConfigurationsFromServer().getPreviewLength();
        ...
        if (modelLayerFactory.getSystemConfigs().getIcapScanActivated()) {
            ...
            if (StringUtils.isBlank(icapServerUrl)) {
                ...
                try {
                    ...
                    final IcapResponse icapResponse = new IcapClient(URI.create(icapServerUrl), icapResponseTimeout).scanFile(filename, fileContent, constellioServerHostname, icapPreviewLength);
                    ...
                    final IcapException.ThreatFoundException threatFoundException = new IcapException.ThreatFoundException(icapResponse.getThreatDescription(), filename);
                    LOGGER.info(threatFoundException.getMessage() + " " + threatFoundException.getThreatName() + " - " + filename);
                    throw threatFoundException;
                } catch (final IOException e) {
                    final IcapException.CommunicationFailure communicationFailureException = new IcapException.CommunicationFailure(e, filename);
