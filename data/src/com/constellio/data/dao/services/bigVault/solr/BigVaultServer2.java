...
public class BigVaultServer2 implements Cloneable {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(BigVaultServer2.class);
	...
	public BigVaultServer2(String name, BigVaultLogger bigVaultLogger, SolrServerFactory solrServerFactory
	...
	public SolrDocument realtimeGet(String id)
		...
		try {
			...
			throw new CouldNotExecuteQuery("realtime get of " + id, e);
		}
	...
	public List<SolrDocument> realtimeGet(List<String> ids)
		...
		try {
			...
			throw new CouldNotExecuteQuery("realtime get of " + ids, e);
		}
	...
	private QueryResponse handleQueryException(SolrParams params, int currentAttempt, Exception solrServerException)
		...
		if (currentAttempt < maxFailAttempt) {
			...
			throw new CouldNotExecuteQuery("query", params, solrServerException);
		}
	}
	...
	TransactionResponseDTO tryAddAll(BigVaultServerTransaction transaction, int currentAttempt)
			throws OptimisticLocking, CouldNotExecuteQuery {
		if (!transaction.getUpdatedDocuments().isEmpty() || !transaction.getNewDocuments().isEmpty()
			...
			try {
				...
				throw new CouldNotExecuteQuery(
	...
	private TransactionResponseDTO handleRemoteSolrExceptionWhileAddingRecords(BigVaultServerTransaction transaction,
		...
		if (code == HTTP_ERROR_409_CONFLICT) {
			...
			throw new BadRequest(transaction, exception);
	...
	private TransactionResponseDTO handleOptimisticLockingException(Exception optimisticLockingException)
		...
		//					solrServerException);
		//		}
		throw new OptimisticLocking(optimisticLockingException);
	}
	...
	@Override
	public BigVaultServer2 clone() {
		return new BigVaultServer2(name, bigVaultLogger, solrServerFactory, extensions, this.listeners);
	}
